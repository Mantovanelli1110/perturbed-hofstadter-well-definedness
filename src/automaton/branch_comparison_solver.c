...
static unsigned short SUPPORT_BRANCH1[MAX_CONTEXTS];
static unsigned short SUPPORT_BRANCH2[MAX_CONTEXTS];
...
static void run_branch(int ctx_id, int state_idx, unsigned short out_support[]) {
    ...
    print_domains("Domains after fixing root and propagating:", dom);
    ...
    printf("\nSupported values under this root choice:\n");
    for (int i = 0; i < CTX_COUNT; i++) {
        printf("ctx %2d: (%s,pos=%d,debt=%d) -> ",
               i, rcode_name(CTX[i].rho), CTX[i].pos, CTX[i].debt);
...
static void compare_branches(const unsigned short a[], const unsigned short b[]) {
    printf("\n====================================\n");
    printf("Branch comparison: intersection/diff\n");
    printf("====================================\n");

    for (int i = 0; i < CTX_COUNT; i++) {
        unsigned short inter = (unsigned short)(a[i] & b[i]);
        unsigned short only_a = (unsigned short)(a[i] & (unsigned short)(~b[i]));
        unsigned short only_b = (unsigned short)(b[i] & (unsigned short)(~a[i]));
        ...
        printf("    only in S1-branch : ");
        print_domain(only_a);
        printf("\n");

        printf("    only in S2-branch : ");
        print_domain(only_b);
        printf("\n\n");
    }
}
...
int main(int argc, char **argv) {
    ...
    if (!propagate_domains(DOMAIN)) {
        printf("RESULT: inconsistency detected before branching.\n");
        return 0;
    }
...

    {
        ...
        run_branch(ctx0, s1, SUPPORT_BRANCH1);
        run_branch(ctx0, s2, SUPPORT_BRANCH2);
        compare_branches(SUPPORT_BRANCH1, SUPPORT_BRANCH2);
    }

    printf("RESULT: branch comparison finished.\n");
    return 0;
}
