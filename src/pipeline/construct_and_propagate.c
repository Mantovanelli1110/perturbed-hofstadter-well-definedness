...
static void build_real_stabilized_automaton(void) {
    ...
    add_comp_state_if_new(0, 2); /* S0[2] */
    add_comp_state_if_new(1, 0); /* S1[0] */
    add_comp_state_if_new(2, 0); /* S2[0] */
    add_comp_state_if_new(3, 0); /* S3[0] */
    add_comp_state_if_new(4, 0); /* S4[0] */
    add_comp_state_if_new(5, 0); /* S5[0] */
    add_comp_state_if_new(6, 0); /* S6[0] */
    add_comp_state_if_new(6, 2); /* S6[2] */
    add_comp_state_if_new(7, 0); /* S7[0] */
    ...
    const int m = (int)(sizeof(REAL_EDGES) / sizeof(REAL_EDGES[0]));
    for (int i = 0; i < m; i++) {
...
/* ------------------------------------------------------------ */
/* Domain initialization and local-consistency propagation      */
/* ------------------------------------------------------------ */
...
/* Proven from previous run: every debt-2 context is forced to S6[2] */
...
static int propagate_domains(void) {
    int changed = 1;
    ...
    while (changed) {
        ...
        for (int i = 0; i < CTX_COUNT; i++) {
            ...
            for (int s = 0; s < COMP_STATE_COUNT; s++) {
                if (!(DOMAIN[i] & (unsigned short)(1u << s))) continue;
                if (!candidate_has_out_support(i, s, DOMAIN)) continue;
                if (!candidate_has_in_support(i, s, DOMAIN)) continue;
...
static void print_context_summary(const char *title) {
    printf("%s\n", title);
    for (int i = 0; i < CTX_COUNT; i++) {
        printf("ctx %2d: (%c,%c,%c,%s,pos=%d,parity=%d,debt=%d)\n",
               ...
               CTX[i].parity,
               CTX[i].debt);
        printf("    domain size = %d : ", bitcount16(DOMAIN[i]));
...
static void print_forced_and_ambiguous(void) {
    printf("Forced contexts:\n");
    for (int i = 0; i < CTX_COUNT; i++) {
        if (bitcount16(DOMAIN[i]) == 1) {
            printf("  ctx %2d -> ", i);
            print_comp_state(firstbit16(DOMAIN[i]));
            printf("\n");
        }
    }

    printf("\nAmbiguous debt-0 contexts:\n");
    for (int i = 0; i < CTX_COUNT; i++) {
        if (CTX[i].debt != 0) continue;
        if (bitcount16(DOMAIN[i]) > 1) {
            printf("  ctx %2d (%s,pos=%d) : ", i, rcode_name(CTX[i].rho), CTX[i].pos);
            print_domain(DOMAIN[i]);
            printf("\n");
        }
    }
...
static int is_core_ctx(int idx) {
    return idx == 0 || idx == 2 || idx == 14 || idx == 24;
}

static void print_one_ctx_brief(int i) {
    printf("  ctx %d: (%c,%c,%c,%s,pos=%d,parity=%d,debt=%d)\n",
           i,
           state_to_char(CTX[i].xm1),
           state_to_char(CTX[i].x0),
           state_to_char(CTX[i].xp1),
...
static void dump_core_psi_edges(void) {
    ...
    printf("========================================================\n");
    printf("Exact induced Psi-edge list on the 4-core {0,2,14,24}\n");
    printf("========================================================\n");
    printf("Core context descriptions:\n");
    print_one_ctx_brief(0);
    print_one_ctx_brief(2);
    print_one_ctx_brief(14);
    print_one_ctx_brief(24);
    printf("\n");
    ...
    for (int i = 0; i < PSI_COUNT; i++) {
        ...
        if (is_core_ctx(s) && is_core_ctx(t)) {
            printf("CORE_PSI: %d -> %d\n", s, t);
            core_edge_count++;
        }
    }
...
int main(void) {
    ...
    print_context_summary("Initial debt-matching domains:");

    force_debt2_to_S6_2();
    print_context_summary("After forcing all debt-2 contexts to S6[2]:");

    if (!propagate_domains()) {
        printf("RESULT: inconsistency detected during local propagation.\n");
        return 0;
    }

    print_context_summary("After iterated local consistency propagation:");
    print_forced_and_ambiguous();

    printf("RESULT: local propagation completed successfully.\n");
    return 0;
}
